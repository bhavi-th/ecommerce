...
import { Mail, Lock, Eye, EyeOff, ShoppingBag, ArrowRight, Chrome, Github } from "lucide-react"
...
const Login = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background via-background to-primary/5 p-2 sm:p-6 md:p-8 relative overflow-hidden">
        ...
        <div className={`relative bg-card/80 backdrop-blur-xl border border-border/50 rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8 shadow-xl transition-all duration-500 hover:shadow-2xl hover:shadow-primary/10 ${cardVisible ? 'animate-scale-in opacity-100' : 'opacity-0 scale-95'}`} ref={cardRef}>
          ...
          <div className="relative">
            <div className="text-center mb-6 sm:mb-8">
              ...
              <p className="text-muted-foreground text-sm sm:text-base">Sign in to continue your journey</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
              ...
              <div className={`flex items-center justify-between text-xs sm:text-sm transition-all duration-500 ${cardVisible ? 'animate-slide-up opacity-100' : 'opacity-0 translate-y-4'}`} style={{ animationDelay: '0.3s' }}>
                <label className="flex items-center gap-2 cursor-pointer group">
                  <input type="checkbox" className="rounded border-border bg-muted focus:ring-primary" />
              ...
              <div className={`relative transition-all duration-500 ${cardVisible ? 'animate-slide-up opacity-100' : 'opacity-0 translate-y-4'}`} style={{ animationDelay: '0.35s' }}>
                <div className="absolute inset-0 flex items-center">
                  <div className="w-full border-t border-border" />
                </div>
                <div className="relative flex justify-center text-xs sm:text-sm">
                  <span className="bg-card px-2 sm:px-4 text-muted-foreground">Or continue with</span>
                </div>
              </div>

              {/* Social Login Buttons */}
              <div className={`grid grid-cols-2 gap-3 sm:gap-4 transition-all duration-500 ${cardVisible ? 'animate-slide-up opacity-100' : 'opacity-0 translate-y-4'}`} style={{ animationDelay: '0.4s' }}>
                <Button
                  type="button"
                  variant="outline"
